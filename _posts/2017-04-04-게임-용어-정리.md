...
- TXAA (Temporal Approximate Anti Aliasing) - MSAA와 주로 비교 되는것으로픽셀의 경계를 넘어 넓은 범위를 읽어 평균화하는 처리를 하기때문에 좀더 `희미하고 자연스러워 보이는 효과`를 얻을 수 있다. (GPU가 수행하는 고도의 처리능력을 필요로하므로 비교적 최신 게임에 사용됨)
